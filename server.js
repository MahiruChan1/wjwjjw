...
// ==========================================
// 7. PYTHON GENERATOR (createpy.js)
// ==========================================
app.post('/api/python', async (req, res) => {
    const { description } = req.body;
    
    if (!description) return res.status(400).json({ message: "Deskripsi diperlukan" });

    // Payload sesuai script asli kamu
    const payload = {
    ...
    try {
        const response = await axios.post("https://aifreeforever.com/api/generate-python-code-ai", payload, {
            headers: {
                'Content-Type': 'application/json',
        ...
        if (response.data.success) {
            ...
            res.json({ success: false, message: "Gagal generate code." });
        }
    } catch (error) {
